...
#
# Works out this machine's IP addresses. Sourced by create_user.sh and
# existing_user.sh.
#####################################################
...
if [ -z "${PUBLIC_IPV6:-}" ]; then
	...
	if [[ -z "${DEFAULT_PUBLIC_IPV6:-}" && -n "$GUESSED_IP" ]]; then
		...
		DEFAULT_PUBLIC_IPV6=$(get_default_privateip 6)
	fi
	...
	if [[ -z "${PUBLIC_IPV6:-}" && $MATCHED == 0 ]]; then
		...
		if [ "${PUBLIC_IPV6_EXITCODE:-0}" -ne 0 ]; then
			# user hit ESC/cancel
			exit
		fi
	fi
...
# Get the IP address of the local network interface that is connected to the
# Internet. We need this when we want to have services bind only to the
# public network interface (not loopback, not tunnel interfaces).
